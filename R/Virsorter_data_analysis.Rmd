...
refseq_by_prophage <- refseq_data %>%
  ...
  # Adds IMG.Scaffold.ID column to datatable by extracting it from the scaffold ID
  # Will allow this data to be combined with metadata
  # Removes prefix
  mutate(IMG.Scaffold.ID = gsub("VIRSorter_", "", Contig_id)) %>%
  # Removes suffix
  mutate(IMG.Scaffold.ID = gsub("\\_.*", "", IMG.Scaffold.ID)) %>%
